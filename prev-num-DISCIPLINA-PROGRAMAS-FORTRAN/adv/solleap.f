       ...
       do i=1,im
       x=(i-1)*100
       temp(i,l)=tzero*sim((2.*pi/ehle)*real(x))
    !   write(*,*)'x(',i,')=',x,'   t=',temp(i,l)
       enddo
       
       l=2;tempo=(l-1)*10
    !   write(*,*)'tempo(',l-1,'):',tempo,'s',tempo/3600,'h'
        i=im;temp(im,l)=temp(im,l-1)-real(u*delt)*((temp(2,l-1)-temp(im-1,l-1))/real(2*delx))
       temp(1,l)=temp(im,l)
       do i=2, im-1
       ...
       open(1,file='solleap1h.bin',status='unknow',form='unformatted',access='direct',action='write',recl=4*im*25)
       write(1,rec=1)temp2
       close(1)
       ...
       open(1,file='solleap2m.bin',status='unknow',form='unformatted',access='direct',action='write',recl=4*im*10)
       write(1,rec=1)temp3
       close(1)
