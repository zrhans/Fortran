       ...
       integer, parameter :: im=101, mt=8641, delt=10, u=8, delx=100
    ...
    !   write(*,*)'tempo(',l-1,'):',tempo,'s',tempo/3600,'h'
     ...
     & ((temp(2,l-1)-temp(im-1,l-1))/real(2*delx))
       temp(1,l)=temp(im,l)
       do i=2, im-1
       temp(i,l)=temp(i,l-1)-real(u*delt)*((temp(i+1,l-1)-
     & temp(i-1,l-1))/real(2*delx))
       enddo
       
       do l=3,mt
       tempo=(l-1)*10
       write(*,*)'tempo(',l-1,'):',tempo,'s',tempo/3600,'h'
       i=im;temp(i,l)=temp(i,l-2)-real(2*u*delt)*
     & ((temp(2,l-1)-temp(im-1,l-1))/real(2*delx))
       temp(1,l)=temp(im,l)
       do i=2, im-1
       temp(i,l)=temp(i,l-2)-real(2*u*delt)*((temp(i+1,l-1)-
     & temp(i-1,l-1))/real(2*delx))
       enddo
       enddo
