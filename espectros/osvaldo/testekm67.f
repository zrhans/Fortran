...
C
	PARAMETER(N=16384,NN=179359,KK=1800,K=8192,M=44,MNN=12)
C
        REAL n12(NN,10)
...
c            6= 1.1<z/L
C
        OPEN(UNIT=13,FILE='STATIS.D13',STATUS='UNKNOWN')
C
...
C
      OPEN(UNIT=11,FILE='n12.dat',STATUS='old')
      DO 10 I=1,NN
      READ (11,*,END=10)(n12(I,J),J=1,10)
10    CONTINUE
C
C
...
C
      WIND2D=SQRT(MEDIO(5)**2+MEDIO(6)**2)
        write(15,*)'KTURB=',kturb
        write(15,*)
        write(15,*)'Medias dos dados de entrada'
      write(15,*)medio(5),medio(6),medio(7),medio(8),medio(9),medio(10)
...
C
        IF(MEDIO(6).GT.0.AND.MEDIO(5).GT.0) THEN
         DR=180.+DA
        ENDIF
C
        IF(MEDIO(6).GT.0.AND.MEDIO(5).LT.0) THEN
         DR=180.-DA
        ENDIF
C
        IF(MEDIO(6).LT.0.AND.MEDIO(5).LT.0) THEN
         DR=DA
        ENDIF
C
        IF(MEDIO(6).LT.0.AND.MEDIO(5).GT.0) THEN
         DR=360.-DA
        ENDIF
C
C
...
C
      AT1=ATAN(MEDIO(6)/MEDIO(5))
C
C EFETUA A PRIEMIRA ROTACAO
C
C NO SONICO
        DO I=1,N
      INPUT1=n12(I+KTURB,5)*cos(AT1)+n12(I+KTURB,6)*sin(AT1)
        INPUT2=n12(I+KTURB,6)*COS(AT1)-n12(I+KTURB,5)*SIN(AT1)
          NINPUT(I,5)=INPUT1
        NINPUT(I,6)=INPUT2
          NINPUT(I,7)=n12(I+KTURB,7)
        end do
c
...
C  NO SONICO
        DO I=1,N
        INPUT3=NINPUT(I,5)*cos(AT2)+NINPUT(I,7)*sin(AT2)
        INPUT4=NINPUT(I,7)*COS(AT2)-NINPUT(I,5)*SIN(AT2)
        NNINPUT(I,5)=INPUT3
          NNINPUT(I,6)=NINPUT(I,6)
        NNINPUT(I,7)=INPUT4
        END DO
C
...
166   CONTINUE
C
        NNMEDIO(5)=NNSOMA(5)/N
        NNMEDIO(6)=NNSOMA(6)/N
        NNMEDIO(7)=NNSOMA(7)/N
C
C RECALCULA THE WIND SPEED NO SONICO
      NNWIND2D=SQRT(NNMEDIO(5)**2+NNMEDIO(6)**2)
C
        write(15,*)
...
8992     FORMAT(3X,'<U3D>',5X,'<V3D>',5X,'<W3D>')
        WRITE(15,28113)NNMEDIO(5),NNMEDIO(6),NNMEDIO(7)
28113     FORMAT(1X,F5.2,3X,F5.2,3X,F5.2)
C
CALCULA AS NOVAS COMPONENTES TURBULENTAS
C
        DO I=1,N
        NTURBU(I)=NNINPUT(I,5)-NNMEDIO(5)
        NTURBV(I)=NNINPUT(I,6)-NNMEDIO(6)
        NTURBW(I)=NNINPUT(I,7)-NNMEDIO(7)
      END DO
C
...
C
        DO I=1,N
        INPUT5=NNINPUT(I,6)*cos(AT3)+NNINPUT(I,7)*sin(AT3)
        INPUT6=NNINPUT(I,7)*COS(AT3)-NNINPUT(I,6)*SIN(AT3)
        NNNINPUT(I,5)=NNINPUT(I,5)
        NNNINPUT(I,6)=INPUT5
        NNNINPUT(I,7)=INPUT6
        END DO
C
...
C
        NNNWIND2D=SQRT(NNNMEDIO(5)**2+NNNMEDIO(6)**2)
        write(15,*)
...
C
       WIND3D=SQRT(NNNMEDIO(5)**2+NNNMEDIO(6)**2+NNNMEDIO(7)**2)
C
C
CALCULA AS COMPONENTES TURBULENTAS APOS A ROTACAOA 3D
c
        DO I=1,N
        TURBU(I)=NNNINPUT(I,5)-NNNMEDIO(5)
        TURBV(I)=NNNINPUT(I,6)-NNNMEDIO(6)
        TURBW(I)=NNNINPUT(I,7)-NNNMEDIO(7)
        ENDDO
...
C
          DO I=1,M-1
      WRITE(US,3781)n12(KTURB+1,5),n12(KTURB+1,6),n12(KTURB+1,7),
     &   n12(KTURB+1,8),n12(KTURB+1,9),n12(KTURB+1,10),
...
3781    FORMAT(1X,F3.0,1X,F3.0,1X,F3.0,
     ...
     &  1X,F15.9)
          END DO
C
      WRITE(13,815)n12(KTURB+1,5),n12(KTURB+1,6),n12(KTURB+1,7),
     &   n12(KTURB+1,8),n12(KTURB+1,9), n12(KTURB+1,10),Z3D/MOL,
     &   USTAR,TSTAR,ECT,WU,WV,WT,T0,WIND2D,DR,
     &   DPU,DPV,DPW,DPT,
     &   DISSIPW,DISMEANW,DISSIPU,DISMEANU,DISSIPV,DISMEANV           
C
815   FORMAT(1X,F3.0,1X,F3.0,1X,F3.0,
     &   1X,F3.0,1X,F3.0,1X,F3.0,1X,F7.2, 
