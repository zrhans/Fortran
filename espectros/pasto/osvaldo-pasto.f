...
C 
	PARAMETER(N=32768,NN=846000,KK=3600,K=16384,M=44,MNN=12)
C
	REAL INPUT(NN,22)
...
C
c
C      print*,'calculando os espectros'
      IF(CALESP.EQ.1)THEN
C
...
c
	...
	CALL CROSS(FFT3,FFT8,COQ,QOQ)
	CALL CROSS(FFT5,FFT6,COC,QOC)
C
      DO I=1,K
	SSPCU(I)=SPCU(I+1)
	SSPCV(I)=SPCV(I+1)
...
c
      ...
      CALL BANDAS(HHERTZ,SSPCQ,HZqS,QSMOOTH)
      CALL BANDAS(HHERTZ,SSPCC,HZqS,CSMOOTH)
	CALL BANDAS(HHERTZ,CCOT,HZwtC,WTSMOOTH)
      CALL BANDAS(HHERTZ,CCOC,HZwcC,WCSMOOTH)
      CALL BANDAS(HHERTZ,CCOQ,HZwqC,WQSMOOTH) 
c
C
